...
Do **not** upload runtime data, secrets, or machine-local junk. In particular:

- **`./data/`** — H2 database files and **uploaded avatars**. These can contain personal names, emails, phones, and photos.
- **`.env`** and any real production credentials (this project uses `application.yml` locally; still never commit secrets)
- **`build/`**, **`.gradle/`** — Gradle output and cache
- IDE noise: **`.idea/`** (including `workspace.xml`), **`*.iml`**, **`.vscode/`** local settings
- Logs (`*.log`, `logs/`) and OS files (`.DS_Store`, `Thumbs.db`)
- H2 lock/trace/database files (`*.mv.db`, `*.trace.db`, `*.lock.db`, `*.h2.db`) anywhere they appear

**Do commit** application source (`src/`), `build.gradle`, `settings.gradle`, this README, and the **Gradle wrapper** (`gradle/wrapper/`, `gradlew`, `gradlew.bat`). Those are how someone else reproduces the build.

`.gitignore` is set to ignore the sensitive and generated paths above.
